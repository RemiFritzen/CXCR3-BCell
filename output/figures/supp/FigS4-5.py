...
"""
figS4-5.py — PB vs CSF comparison, Supplementary Figure 5
...
def make_figS4_cxcr3pos_pb_vs_csf() -> None:
    """
    CXCR3+ counterpart to make_figS4_pb_vs_csf() (which covers CXCR3-),
    ...
    make_figS4_pb_vs_csf() (CXCR3-), gives Supplementary Figure 5 full
    ...
    if "HC" in df_sig["group_label"].unique():
        df_sig[df_sig["group_label"] == "HC"].to_csv(
            out_dir / "FigS4_CXCR3pos_PB_vs_CSF_HC_zinc_signature_per_cell.csv",
            index=False
        )
    if "MS" in df_sig["group_label"].unique():
        df_sig[df_sig["group_label"] == "MS"].to_csv(
            out_dir / "FigS4_CXCR3pos_PB_vs_CSF_MS_zinc_signature_per_cell.csv",
            index=False
        )
    ...
    for name, res in [("HC", sig_test_hc), ("MS", sig_test_ms)]:
        pd.DataFrame([{k: v for k, v in res.items() if k != "per_patient_values"}]).to_csv(
            out_dir / f"FigS4_CXCR3pos_PB_vs_CSF_{name}_zinc_signature_patient_level_test.csv",
            index=False,
        )
    ...
    stem = "FigS4_CXCR3pos_PB_vs_CSF_6panel"
    out_png = out_dir / f"{stem}.png"
    out_pdf = out_dir / f"{stem}.pdf"
    print(f"\n✓ Figure saved to {out_png} and {out_pdf}")
