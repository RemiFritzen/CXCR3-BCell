...
PB and CSF are treated in SEPARATE files (FigS7 = PB, FigS7 = CSF) rather
...
TISSUE = "CSF"  # the only thing that differs from FigS7.py (which uses "PB")
...
def _panel_letter(ax: plt.Axes, letter: str) -> None:
    ax.text(-0.15, 1.05, letter, transform=ax.transAxes, **PANEL_KW)
...
def draw_paired_patient_plot_cxcr3(ax, sig_test_result, group_label, letter, signed: bool = True):
    ...
    ax.boxplot(
        [neg_vals, pos_vals], positions=[1, 2], widths=0.5,whis=(0,100),
...
def make_figS7_cxcr3_CSF() -> None:
    ...
    # Unsigned comparison. The a priori sign scheme assumes MTs fall in
    # CXCR3+; where that assumption fails the signed and unsigned scores move
    # in opposite directions, so both are written out. Naming matches the
    # _UNSIGNED_ convention used by Fig2 and FigS4.
    df_sig_unsigned = compute_cell_signature_cxcr3(
        adata, zinc_genes=zinc_genes, gene_signs=None
    )
    for cond in ("HC", "MS"):
        if cond in df_sig_unsigned["condition"].unique():
            df_sig_unsigned[df_sig_unsigned["condition"] == cond].to_csv(
                out_dir / f"FigS7_CXCR3_{TISSUE}_{cond}_zinc_signature_UNSIGNED_per_cell.csv",
                index=False,
            )
    ...
    for name, res in [("HC", sig_test_hc_unsigned), ("MS", sig_test_ms_unsigned)]:
        pd.DataFrame([{k: v for k, v in res.items() if k != "per_patient_values"}]).to_csv(
            out_dir / f"FigS7_CXCR3_{TISSUE}_{name}_zinc_signature_UNSIGNED_patient_level_test.csv",
            index=False,
        )
