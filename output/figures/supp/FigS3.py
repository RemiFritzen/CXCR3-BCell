...
"""
figS3.py — PB + CSF CXCR3- MS vs HC zinc signature (Supplementary Figure 4)
...
def compute_cell_signature(adata, zinc_genes: list[str], tissue: str, gene_signs: dict[str, float] | None = None) -> pd.DataFrame:
    ...
    equally justified for every comparison (see draw_violin/make_figS3_hc_vs_ms_cxcr3neg).
...
def _draw_volcano(ax: plt.Axes, df: pd.DataFrame, title: str, label: str, zinc_color: str) -> None:
    ...
    other = df[~df["is_zinc"]]
    zinc = df[df["is_zinc"]]
    zinc_sig = zinc[zinc["padj_zinc_panel"] < SIG_PADJ_THRESHOLD]
    zinc_ns = zinc[~(zinc["padj_zinc_panel"] < SIG_PADJ_THRESHOLD)]
...
def make_figS3_hc_vs_ms_cxcr3neg() -> None:
    ...
    df_sig_pb.to_csv(out_dir / "FigS3_PB_CXCR3neg_zinc_signature_per_cell.csv", index=False)
    df_sig_csf.to_csv(out_dir / "FigS3_CSF_CXCR3neg_zinc_signature_per_cell.csv", index=False)
    ...
    pd.DataFrame([{k: v for k, v in sig_test_pb.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS3_PB_CXCR3neg_zinc_signature_patient_level_test.csv", index=False
    )
    pd.DataFrame([{k: v for k, v in sig_test_csf.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS3_CSF_CXCR3neg_zinc_signature_patient_level_test.csv", index=False
    )
    ...
    confound_pb["per_dataset"].to_csv(
        out_dir / "FigS3_PB_CXCR3neg_dataset_confounding_check.csv", index=False
    )
    confound_csf["per_dataset"].to_csv(
        out_dir / "FigS3_CSF_CXCR3neg_dataset_confounding_check.csv", index=False
    )
    pd.DataFrame([
    ...
    ]).to_csv(out_dir / "FigS3_dataset_confounding_summary.csv", index=False)

    # Unsigned (plain-mean) version of the same signature, computed
    # alongside the sign-corrected one for transparency -- see
    # make_figS3_hc_vs_ms_cxcr3neg's supplementary figure below and the discussion of
    ...
    df_sig_pb_unsigned.to_csv(out_dir / "FigS3_PB_CXCR3neg_zinc_signature_UNSIGNED_per_cell.csv", index=False)
    df_sig_csf_unsigned.to_csv(out_dir / "FigS3_CSF_CXCR3neg_zinc_signature_UNSIGNED_per_cell.csv", index=False)
    ...
    pd.DataFrame([{k: v for k, v in sig_test_pb_unsigned.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS3_PB_CXCR3neg_zinc_signature_UNSIGNED_patient_level_test.csv", index=False
    )
    pd.DataFrame([{k: v for k, v in sig_test_csf_unsigned.items() if k != "per_patient_values"}]).to_csv(
        out_dir / "FigS3_CSF_CXCR3neg_zinc_signature_UNSIGNED_patient_level_test.csv", index=False
    )
    print(
    ...
    stem = "FigS3_CXCR3neg_PB_CSF_MS_vs_HC_4panel"
    ...
    _save_single_volcano(
        ...
        out_png=supp_dir / "FigS3_supp_PB_volcano.png",
        out_pdf=supp_dir / "FigS3_supp_PB_volcano.pdf",
    )
    _save_single_volcano(
        df_csf,
        ...
        out_png=supp_dir / "FigS3_supp_CSF_volcano.png",
        out_pdf=supp_dir / "FigS3_supp_CSF_volcano.pdf",
    )
    ...
    fig_supp_sig.suptitle(
        "Supplementary: unsigned zinc/MT signature\n"
        "(compare against main Fig 2C/D, which use the sign-corrected version)",
        fontsize=10, y=1.04,
    )
    fig_supp_sig.tight_layout()
    fig_supp_sig.savefig(supp_dir / "FigS3_supp_unsigned_signature_violin.png", dpi=300, bbox_inches="tight")
    fig_supp_sig.savefig(supp_dir / "FigS3_supp_unsigned_signature_violin.pdf", bbox_inches="tight")
    plt.close(fig_supp_sig)

    df_pb.to_csv(out_dir / "FigS3_PB_CXCR3neg_MS_vs_HC_full_DE.csv", index=False)
    df_csf.to_csv(out_dir / "FigS3_CSF_CXCR3neg_MS_vs_HC_full_DE.csv", index=False)
    pd.DataFrame({"gene": pb_bar_genes}).to_csv(out_dir / "FigS3_PB_CXCR3neg_zinc_bar_genes.csv", index=False)
    pd.DataFrame({"gene": csf_bar_genes}).to_csv(out_dir / "FigS3_CSF_CXCR3neg_zinc_bar_genes.csv", index=False)
